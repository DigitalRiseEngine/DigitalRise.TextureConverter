	...
	partial class TextureHelper
	{
		...
		private class LanczosFilter : Filter
		{
			...
			protected override float Evaluate(float x)
			{
				...
				return x < 3.0f ? Sinc(MathUtility.Pi * x) * Sinc(MathUtility.Pi * x / 3.0f) : 0.0f;
			}
		}
		...
		private class KaiserFilter : Filter
		{
			...
			protected override float Evaluate(float x)
			{
				float sinc_value = Sinc(MathUtility.Pi * x * _stretch);
				float t = x / Width;
				if ((1 - t * t) >= 0)
		...
		private class GaussianFilter : Filter
		{
			...
			protected override float Evaluate(float x)
			{
				return 1.0f / (float)Math.Sqrt(2 * MathUtility.Pi * _variance) * (float)Math.Exp(-x * x / (2 * _variance));
			}
		}
		#endregion
...
#endif
					{
						...
						for (int x = 0; x < tmpImage4F.Width; x++)
						{
							...
							for (int i = 0; i < kernelX.WindowSize; i++)
							{
								...
								//if (MathUtility.IsNaN(color.X) || MathUtility.IsNaN(color.Y) || MathUtility.IsNaN(color.Z) || MathUtility.IsNaN(color.W)
								...
								//if (MathUtility.IsNaN(sum.X) || MathUtility.IsNaN(sum.Y) || MathUtility.IsNaN(sum.Z) || MathUtility.IsNaN(sum.W)
